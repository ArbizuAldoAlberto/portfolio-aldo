import { useTranslations } from 'next-intl'

export default function Footer() {
  const t = useTranslations('Hero') // We can use shared translations or static text
  const currentYear = new Date().getFullYear()

  return (
    <footer className="border-t border-[var(--color-space-border)] bg-[var(--color-space-black)] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex flex-col items-center md:items-start">
          <span className="font-serif font-bold text-2xl text-white">ARBIZU LABS</span>
          <span className="font-space text-xs text-[var(--color-mist-gray)]/60 mt-1 uppercase tracking-widest">
            B2B Mobile & SaaS Architect
          </span>
        </div>

        <div className="flex items-center gap-6">
          <a href="https://arbizualdo.gumroad.com" target="_blank" rel="noreferrer" className="font-space text-xs uppercase tracking-widest text-[var(--color-mist-gray)] hover:text-[var(--color-orbital-teal)] transition-colors border border-[var(--color-space-border)] px-3 py-1 rounded-full bg-white/5">
            Gumroad Store
          </a>
          <a href="https://github.com/ArbizuAldoAlberto" target="_blank" rel="noreferrer" className="text-[var(--color-mist-gray)] hover:text-[var(--color-orbital-teal)] transition-colors">
            <span className="sr-only">GitHub</span>
            <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
            </svg>
          </a>
          <a href="https://linkedin.com/in/aldoarbizu" target="_blank" rel="noreferrer" className="text-[var(--color-mist-gray)] hover:text-[var(--color-electric-purple)] transition-colors">
            <span className="sr-only">LinkedIn</span>
            <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" />
            </svg>
          </a>
        </div>
      </div>
      
      <div className="max-w-7xl mx-auto mt-8 border-t border-[var(--color-space-border)]/50 pt-8 flex flex-col md:flex-row justify-between items-center">
        <p className="text-[10px] font-mono text-[var(--color-mist-gray)]/40 text-center md:text-left">
          &copy; {currentYear} Arbizu Labs. All rights reserved. <br/>
          Built with React Server Components & Offline-First architecture.
        </p>
        <p className="text-[10px] font-space text-[var(--color-mist-gray)]/40 text-center md:text-right mt-4 md:mt-0 uppercase tracking-widest">
          NEXUS System 100% Operational
        </p>
      </div>
    </footer>
  )
}
